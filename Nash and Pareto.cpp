...
void balance_Nesh(int N) //ðàâíîâåñèå ïî Íýøó
{
	size_t flag = 0;
	size_t flag2 = 0;
	...
	for (auto i = 0; i < N; ++i)
	{
		for (auto j = 0; j < N; ++j)
		{
			...
			flag2 = 0;
			if (i > 0 && j == 0 && i < N - 1 && (matrix_for_player_B(i, j) >= matrix_for_player_B(i, j + 1))) //i>1 è i<N-1 è j=0
			{
			...
			flag2 = 0;
			if (i > 0 && j == N - 1 && i < N - 1 && (matrix_for_player_B(i, j) >= matrix_for_player_B(i, j - 1))) //i>1 è i<N-1 è j=N-1
			{
...
void balance_Pareto(int N) //ðàâíîâåñèå ïî Ïàðåòî
{
